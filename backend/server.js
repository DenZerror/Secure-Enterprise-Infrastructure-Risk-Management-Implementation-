...
// ─────────────────────────────────────────────────────────────────────────────
// ADD THIS ROUTE to backend/server.js  — paste it right after the /send route
// (after line: res.json({ status: 'Secure message received and verified' });  )
// ─────────────────────────────────────────────────────────────────────────────

// ── INSECURE ROUTE (for educational demo only) ──
// Accepts plaintext message with no encryption, no signature, no JWT
app.post('/send-plain', (req, res) => {
  try {
    ...
    console.warn(`⚠️  [INSECURE] [${timestamp}] ${sender}: "${message}"  ← PLAINTEXT, NO ENCRYPTION`);

    // Save plaintext directly (no encryption at rest either, to illustrate the risk)
    const path = require('path');
    const fs   = require('fs');
    const INSECURE_LOG = path.join(__dirname, 'data', 'insecure_messages.json');
    const dir  = path.dirname(INSECURE_LOG);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const existing = fs.existsSync(INSECURE_LOG)
      ? JSON.parse(fs.readFileSync(INSECURE_LOG, 'utf8'))
      : [];
    existing.push({ sender, timestamp, message }); // ← stored in plain text!
    fs.writeFileSync(INSECURE_LOG, JSON.stringify(existing, null, 2));

    res.json({
      status    : 'Insecure message received (no encryption, no auth)',
      warning   : 'This message was transmitted and stored in PLAINTEXT',
      plaintext : message, // echo back so frontend can show it was readable
    });
  } catch (err) {
...
// ── INSECURE MESSAGES fetch ──
app.get('/messages-plain', (req, res) => {
  try {
    const path = require('path');
    const fs   = require('fs');
    const INSECURE_LOG = path.join(__dirname, 'data', 'insecure_messages.json');
    if (!fs.existsSync(INSECURE_LOG)) return res.json([]);
    res.json(JSON.parse(fs.readFileSync(INSECURE_LOG, 'utf8')));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
